...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const skal = x1 * x2 + y1 * y2; // скалярное произведение векторов
  const mod1 = Math.sqrt(x1 ** 2 + y1 ** 2); // модули векторов
  const mod2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(skal / (mod1 * mod2)); // вычисляем арккосинус
}
...
function getLastDigit(value) {
  return value % 10; // отбрасывается все кроме остатка от деления на 10.
}
...
function getParallelepipedDiagonal(a, b, c) {
  // корень квадратный из суммы квадратов чисел
...
function toNumber(value, def) {
  return Number(value) || def; // вернем преобразованное число или исх. значение
}
...
function getSumOfDigits(num) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    sum += parseInt(arr[i], 10); // преобразуем каждую цифру в число и добавляем его к сумме.
  }
  return sum;
}
...
function isNumber(number) {
  return Number.isFinite(number); // статич. метод для опред-ия конечное число ли. Отсеивает infinity и Nan.
}
...
function getFloatOnString(str) {
  return Number.parseFloat(str); // Nan возвращается по умолчанию если преобр. нельзя.
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Number(x1 + x2 + x3).toFixed(2); // устраняет проблему слож-ия чисел с плав. запятой
}
...
function getRandomInteger(min, max) {
  return Math.floor(Math.random() * (max - min) + min); // увелич. диапазон до max и сдвигаем на min
}
